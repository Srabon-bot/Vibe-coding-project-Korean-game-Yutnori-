...
export function WinModal() {
  ...
  function handlePlayAgain() {
    playSound('click');
    restart({ piecesPerPlayer });
  }

  return (
    <div className="modal-backdrop">
      <div className="panel ornate-frame" style={{ padding: '32px 40px', textAlign: 'center', maxWidth: 420 }}>
        <div
          style={{
            width: 20,
            height: 20,
        ...
        <p className="panel-title" style={{ fontSize: '1.8rem', margin: '0 0 4px' }}>
          {t('win.title', { nickname: winnerNickname ?? '' })}
        </p>
        <p style={{ opacity: 0.8, margin: '0 0 20px', fontSize: '0.9rem' }}>{t('win.subtitle')}</p>
        <div className="taegeuk-divider" />
        <button className="btn" style={{ marginTop: 16 }} onClick={handlePlayAgain}>
          {t('win.playAgain')}
        </button>
      </div>
    </div>
